...
do
  ...
  -- Helper function to find the most likely skill level
  local function FindSkillLevel(sp,skill)
  ...
  RAIL.Event["AI CYCLE"]:Register(-40,                  -- Priority
    ...
    local self = RAIL.Self

    -- Generate a new table for the actor
    RAIL.Self.SkillState = {
      [key] = {
        ["actor"] = self,
    ...
    RAIL.Self.SkillState.Update[key] = RAIL.Self.SkillState
    RAIL.Self.SkillState.Callbacks[key] = RAIL.Self.SkillState

    -- Set metatables
    setmetatable(RAIL.Self.SkillState,mt)
    setmetatable(RAIL.Self.SkillState.Update,update_mt)
    setmetatable(RAIL.Self.SkillState.Callbacks,callbacks_mt)
