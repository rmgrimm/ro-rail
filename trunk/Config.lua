...
-- state files between account ID, homu/merc, and RAIL revision number.
--
-- The following patterns are allowed in the file specification:
--  {1} - This will expand to the account ID number
...
--  "RAIL_State.{2}.lua",
--  "RAIL_State.{1}.{2}.lua"
-- These values will be compatible with older versions of RAIL. The first will
-- differentiate only by homunculi and mercenary; that is, all homunculi
-- regardless of account will use the same settings. Similarly, all
-- mercenaries will also use the same settings. However, homunculi and
-- mercenaries will use different settings, even if used simultaneously. The
-- second example will separate all settings between all accounts and AI
-- types. This is useful for specialized setups, or when running multiple
-- copies of Rampage AI simultaneously.
...
-- Ragnarok Renewal brought some subtle changes to homunculus. Set the following
-- option to true if you are playing on a renewal server
RAIL.Renewal = true
